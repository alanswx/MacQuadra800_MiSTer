...
always @(posedge tb_ap040_program.clk) if(tb_ap040_program.nreset && `C.ce && `C.pipe_cancel) begin
 if(`C.pipe_pc!='h600)$fatal(1,"IRQ did not stop after the store: pc=%h",`C.pipe_pc);
 kills=kills+int'(`C.integer_pipeline.id_v)+int'(`C.integer_pipeline.ex_v);
end
final begin
 if(injections!=3 || kills<3 || stores!=3)$fatal(1,"store IRQ coverage missing");
 $display("STORE IRQ injections=%0d killed=%0d stores=%0d",injections,kills,stores);
end
endmodule
`undef C
