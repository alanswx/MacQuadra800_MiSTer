...
verilator=${CPU_GATE_VERILATOR:-/home/alans/verilator5/bin/verilator}
...
"$verilator" --binary --timing --build-jobs 8 -Wno-fatal --top-module tb_corpus \
...
if rg -q 'FAIL:|dumping partial results' "$out/run.log" ||
   ! rg -q 'CORPUS DONE in [0-9]+ cycles' "$out/run.log"; then
    echo "FAIL: incomplete simulation; inspect $out/run.log" >&2; exit 1
fi
python "$repo/SingleStepTests/gen/score_vs_oracle.py" cpu "$baseline" \
    "$out/results.bin" --flat-env | tee "$out/score.log"
rg -q '^100 rows:' "$out/score.log" || {
