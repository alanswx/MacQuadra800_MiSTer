...
parser=argparse.ArgumentParser(description="Check all short branch conditions after pipeline compare retirement.")
...
(d/'monitor.sv').write_text('`timescale 1ns/1ps\n`define C tb_ap040_program.dut.core\nmodule drain_monitor;\ninteger count=0,taken=0,not_taken=0;\nalways @(posedge tb_ap040_program.clk) if(tb_ap040_program.nreset && `C.ce && `C.pipe_drain && `C.pipe_retire && `C.rd_is_bcc && !`C.pipe_input && !`C.pipe_cancel) begin\n count++;\n if(`C.rd_bcc_taken) taken++; else not_taken++;\nend\nfinal begin\n $display("BRANCH_WB count=%0d taken=%0d not_taken=%0d",count,taken,not_taken);\n if(count!=360 || taken==0 || not_taken==0) $fatal(1,"branch final-WB coverage missing");\nend\nendmodule\n')
def run(cmd,name):
...
sources=[r/'rtl/ap68040/tb/tb_ap040_program.v',d/'monitor.sv',exp/'handoff_monitor.sv',args.core.resolve(),rtl/'ap040_regfile.v',exp/'ap040_pipeline_integer.sv',*[rtl/(u+'.v') for u in units]]
