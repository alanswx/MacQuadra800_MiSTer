#!/usr/bin/env bash
# Prepared P136 seed-24 fit wrapper. Not launched.
# Run only after root deliberately applies/promotes the P136 cache patch in THIS
# checkout. It never applies a patch, creates a worktree, or changes production HDL.
set -u
repo=/home/alans/mister/MacQuadra800_MiSTer
fit_out="$repo/scratch/p136_hinted_parallel_span_fit_20260921"
cd "$repo" || exit 1

if pgrep -x 'quartus_.*' >/dev/null 2>&1; then
  echo 'Quartus process already active; refusing to start P136.' >&2
  exit 3
fi
...
mkdir -p "$fit_out"
printf '%s\n' 'P136 development fit: CDROM_OFF and ETHERNET_OFF; no hardware deployment.' > "$fit_out/FEATURES.txt"
fit_commit=$(git rev-parse HEAD)
printf '%s\n' "$fit_commit" > "$fit_out/commit.txt"

# The promoted P136 source must already be in this checkout. The patch file is
...
[[ "$actual_cache" == 654b8750cdafcb19235367b8e7f5ddab8f2d5b8a4896d15b0cd2b53750ec58ba ]] || { echo 'P136 cache is not promoted in this checkout' >&2; exit 2; }
grep -Fxq 'set_global_assignment -name SEED 24' MacQuadra800.qsf || exit 2
...
printf '%s\n' "core=$actual_core pipeline=$actual_pipe cache=$actual_cache seed=24 CDROM_OFF=1 ETHERNET_OFF=1" > "$fit_out/IDENTITY_CHECK.txt"
...
if [[ -f output_files/MacQuadra800.rbf && output_files/MacQuadra800.rbf -nt "$fit_out/start.stamp" ]]; then
  rbf="$fit_out/MacQuadra800_p136_hinted_parallel_span_seed24_${fit_commit:0:7}.rbf"
...
if [[ "$fit_ready" -eq 1 && -n "${QUARTUS_BIN:-}" ]]; then
  "$QUARTUS_BIN/quartus_sta" -t scripts/cpu/timequest_cross_domain.tcl p136_hinted_parallel_span_seed24 > "$fit_out/cross.log" 2>&1
  cross_rc=$?
...
copy_fresh scratch/cross_sys2ram_p136_hinted_parallel_span_seed24.txt "$fit_out/cross_sys2ram.txt"
copy_fresh scratch/cross_ram2sys_p136_hinted_parallel_span_seed24.txt "$fit_out/cross_ram2sys.txt"
...
rg -n 'RAM|M10K|ALM|Total registers|slack|Slack|Critical Warning|Error' "$fit_out"/*.rpt "$fit_out"/*.summary "$fit_out"/worst_*.txt > "$fit_out/resource_timing_extract.txt" 2>/dev/null || true
