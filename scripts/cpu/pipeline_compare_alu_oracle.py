...
def emit(kind,*args):
 global pc,ccr
 if kind not in ('move','shift','lea','tst'):return base_emit(kind,*args)
 at=pc;pc+=2;ext=0
 if kind=='tst':
  dest,size=args;word=0x4a00+(size<<6)+dest
  mask=(1 << (8 << size))-1;value=regs[dest]&mask
  ccr=(ccr&16)|(8 if value&(1 << ((8 << size)-1)) else 4 if value==0 else 0)
 elif kind=='move':
...
for value in (0,0x80000000,0x80008000,0x80000080,0xffffffff,0x7f00ff01):
 for dest in range(8):
  constant(dest,value)
  for size in range(3):emit('tst',dest,size)
...
for base in range(8):
 for size in range(3):
  ...
  supported.add(0x4a10+(size<<6)+base)
...
s=(exp/'tb_pipeline_pea.sv').read_text().replace('.ENABLE_PEA(1))','.ENABLE_PEA(1), .ENABLE_SHIFTS(1), .ENABLE_DISP_LEA(1), .ENABLE_COMPARE(1))')
