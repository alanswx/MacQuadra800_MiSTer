#!/usr/bin/env bash
# Prepared P150 seed-25 fit wrapper. NOT LAUNCHED.
...
fit_out="$repo/scratch/p150_routing_seed25_fit_20260921"
cd "$repo" || exit 1

if pgrep -x 'quartus_.*' >/dev/null 2>&1; then
  echo 'Quartus process already active; refusing to start P150.' >&2
  exit 3
fi
...
mkdir -p "$fit_out"
printf '%s\n' 'P150 development fit: CDROM_OFF and ETHERNET_OFF; no hardware deployment.' > "$fit_out/FEATURES.txt"
...
if [[ -f output_files/MacQuadra800.rbf && output_files/MacQuadra800.rbf -nt "$fit_out/start.stamp" ]]; then
  rbf="$fit_out/MacQuadra800_p150_routing_seed25_seed25_${fit_commit:0:7}.rbf"
...
if [[ "$fit_ready" -eq 1 && -n "${QUARTUS_BIN:-}" ]]; then
  "$QUARTUS_BIN/quartus_sta" -t scripts/cpu/timequest_cross_domain.tcl p150_routing_seed25_seed25 > "$fit_out/cross.log" 2>&1
  cross_rc=$?
...
copy_fresh scratch/cross_sys2ram_p150_routing_seed25_seed25.txt "$fit_out/cross_sys2ram.txt"
copy_fresh scratch/cross_ram2sys_p150_routing_seed25_seed25.txt "$fit_out/cross_ram2sys.txt"
