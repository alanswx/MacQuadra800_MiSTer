...
// Measured edge counts; remain zero until a qualified overlap is observed.
integer admission_snoop_cases = 0, lookup_snoop_cases = 0, paused_snoop_cases = 0;
always @(posedge clk) begin
  trace_cycle = trace_cycle + 1;
`ifdef P122_CANDIDATE_COVERAGE
  if (trace_arm && (c_req || c_ack || s_stb || m_req || dut.snoop_wr || dut.fast_ifetch_idle || dut.fast_ifetch_look))
    $display("SNOOP_TRACE cyc=%0d ce=%b c_req=%b c_ack=%b instr=%b addr=%h s_stb=%b s_addr=%h snoop_wr=%b look_snooped=%b snoop_look_row=%b cst=%0d fast_idle=%b fast_look=%b m_req=%b m_ack=%b mread=%0d", trace_cycle, ce, c_req, c_ack, c_instr, c_addr, s_stb, s_addr, dut.snoop_wr, dut.look_snooped, dut.snoop_look_row, dut.cst, dut.fast_ifetch_idle, dut.fast_ifetch_look, m_req, m_ack, mread_count);
`else
  if (trace_arm && (c_req || c_ack || s_stb || m_req || dut.snoop_wr))
    $display("SNOOP_TRACE cyc=%0d ce=%b c_req=%b c_ack=%b instr=%b addr=%h s_stb=%b s_addr=%h snoop_wr=%b look_snooped=%b snoop_look_row=%b cst=%0d m_req=%b m_ack=%b mread=%0d", trace_cycle, ce, c_req, c_ack, c_instr, c_addr, s_stb, s_addr, dut.snoop_wr, dut.look_snooped, dut.snoop_look_row, dut.cst, m_req, m_ack, mread_count);
`endif
end
...
integer fast_look_snoop_edges = 0, fast_look_after_ce_resume = 0;
reg paused_snoop_seen = 0;
...
`ifdef P122_CANDIDATE_COVERAGE
    if (s_stb && c_req && c_instr && ce && dut.fast_ifetch_look) fast_look_snoop_edges = fast_look_snoop_edges + 1;
    if (paused_snoop_seen && ce && c_req && c_instr && dut.fast_ifetch_look) fast_look_after_ce_resume = fast_look_after_ce_resume + 1;
`endif
    if (s_stb && c_req && c_instr && ce && dut.cst == 3'd0 && s_addr[31:4] == c_addr[31:4]) admission_snoop_cases = admission_snoop_cases + 1;
    if (s_stb && c_req && c_instr && ce && dut.cst == 3'd1 && s_addr[31:4] == c_addr[31:4]) lookup_snoop_cases = lookup_snoop_cases + 1;
    if (s_stb && c_req && c_instr && !ce && dut.cst == 3'd1 && s_addr[31:4] == c_addr[31:4]) begin
        paused_snoop_cases = paused_snoop_cases + 1;
        paused_snoop_seen = 1;
    end
end
...
`endif
    // Instruction-snoop coincidence probe: preserve a warm I-line, change
    // backing memory, clear only the line buffer, then overlap an I-read
    // with a same-line external snoop.  Record stale-vs-fresh explicitly.
    c_instr = 1; ce_run = 1;
    mem[32'hF500>>2] = 32'h1111_2222;
    cpu_read_count_sized(32'h0000_F500, 2'b10, d, fill_cycles);
    cinv_req = 1; cinv_ic = 0; cinv_dc = 1;
    @(negedge clk); while (!cinv_done) @(posedge clk);
    cinv_req = 0; cinv_dc = 0;
    mem[32'hF500>>2] = 32'h3333_4444;
    trace_arm = 1;
    fork
      begin
        cpu_read_count_sized(32'h0000_F500, 2'b10, d, normal_cycles);
        if (d !== 32'h1111_2222) $fatal(1, "I-SNOOP stale-line contract violated got=%h expected=11112222", d);
        $display("I-SNOOP PASS preserved_dcache_snoop_old=%h", d);
      end
      begin
        @(negedge clk); s_addr = 32'h0000_F500; s_stb = 1;
        @(negedge clk); s_stb = 0;
      end
    join
    trace_arm = 0;
    // D-only snoop preserves the I-line by contract. Explicit instruction
    // CINV must then force the changed backing word into the I-bank.
    cinv_req = 1; cinv_ic = 1; cinv_dc = 0;
    @(negedge clk); while (!cinv_done) @(posedge clk);
    cinv_req = 0; cinv_ic = 0;
    cpu_read_count_sized(32'h0000_F500, 2'b10, d, normal_cycles);
    if (d !== 32'h3333_4444) $fatal(1, "I-CINV fresh-line contract violated got=%h expected=33334444", d);
    $display("I-CINV PASS fresh=%h", d);
    // Explicit lookup-window D-snoop: a miss remains in C_LOOK long
    // enough for the same-line D snoop to arrive before completion.
    c_instr = 1; ce_run = 1;
    // Warm the I-line, clear only the line buffer with a D-CINV, and change
    // backing memory.  The following access must be a real C_LOOK hit; the
    // unprimed hint is deliberate.  A D-only snoop preserves the old I word.
    mem[32'hF700>>2] = 32'h5555_6666;
    cpu_read_count_sized(32'h0000_F700, 2'b10, d, fill_cycles);
    cinv_req = 1; cinv_ic = 0; cinv_dc = 1;
    @(negedge clk); while (!cinv_done) @(posedge clk);
    cinv_req = 0; cinv_dc = 0;
    mem[32'hF700>>2] = 32'hAAAA_BBBB;
    // Deliberately leave the registered hint unrelated; do not drive the
    // internal hint model directly.
    @(negedge clk); c_req = 0; c_addr = 32'hDEAD_0000;
    repeat (2) @(posedge clk);
    fork
      begin
        cpu_read_count_sized(32'h0000_F700, 2'b10, d, normal_cycles);
        if (d !== 32'h5555_6666) $fatal(1, "lookup-snoop warm read got %h", d);
      end
      begin
        integer g_lookup;
        g_lookup = 0;
        while (!(c_req && dut.cst == 3'd1) && g_lookup < 100) begin @(negedge clk); g_lookup = g_lookup + 1; end
        if (g_lookup >= 100) $fatal(1, "warm lookup C_LOOK not observed");
        s_addr = 32'h0000_F700; s_stb = 1;
        @(negedge clk); s_stb = 0;
      end
    join
    cinv_req = 1; cinv_ic = 1; cinv_dc = 0;
    ...
    cinv_req = 0; cinv_ic = 0;
    cpu_read_count_sized(32'h0000_F700, 2'b10, d, normal_cycles);
    if (d !== 32'hAAAA_BBBB) $fatal(1, "lookup-snoop I-CINV fresh got %h", d);

    // Repeat the warm C_LOOK hit with CE paused.  Drive the snoop on a
    // negedge while CE is already low, then resume after the pulse so no
    // assertion depends on a posedge race.
    mem[32'hF800>>2] = 32'h7777_8888;
    cpu_read_count_sized(32'h0000_F800, 2'b10, d, fill_cycles);
    cinv_req = 1; cinv_ic = 0; cinv_dc = 1;
    @(negedge clk); while (!cinv_done) @(posedge clk);
    cinv_req = 0; cinv_dc = 0;
    mem[32'hF800>>2] = 32'hCCCC_DDDD;
    // Deliberately leave the registered hint unrelated; do not drive the
    // internal hint model directly.
    @(negedge clk); c_req = 0; c_addr = 32'hDEAD_0000;
    repeat (2) @(posedge clk);
    fork
      begin
        cpu_read_count_sized(32'h0000_F800, 2'b10, d, normal_cycles);
        if (d !== 32'h7777_8888) $fatal(1, "paused-snoop warm read got %h", d);
      end
      begin
        integer g_paused;
        g_paused = 0;
        while (!(c_req && dut.cst == 3'd1) && g_paused < 100) begin @(negedge clk); g_paused = g_paused + 1; end
        if (g_paused >= 100) $fatal(1, "paused warm lookup C_LOOK not observed");
        ce_run = 0;
        @(negedge clk); s_addr = 32'h0000_F800; s_stb = 1;
        @(negedge clk); s_stb = 0;
    ...
    cinv_req = 0; cinv_ic = 0;
    cpu_read_count_sized(32'h0000_F800, 2'b10, d, normal_cycles);
    if (d !== 32'hCCCC_DDDD) $fatal(1, "paused-snoop I-CINV fresh got %h", d);
    c_instr = 0;

`ifdef P122_CANDIDATE_COVERAGE
    if (fast_look_snoop_edges == 0 || fast_look_after_ce_resume == 0)
        $fatal(1, "candidate fast C_LOOK overlap missing snoop=%0d after_ce_resume=%0d", fast_look_snoop_edges, fast_look_after_ce_resume);
`endif
    if (admission_snoop_cases == 0 || lookup_snoop_cases == 0 || paused_snoop_cases == 0)
        $fatal(1, "instruction D-snoop windows missing admission=%0d lookup=%0d paused=%0d", admission_snoop_cases, lookup_snoop_cases, paused_snoop_cases);
    $display("I-D-SNOOP WINDOWS admission=%0d lookup=%0d paused=%0d", admission_snoop_cases, lookup_snoop_cases, paused_snoop_cases);
    ...
    $display("IREAD PASS requests=%0d acks=%0d mem_reads=%0d idle_cycles=%0d look_cycles=%0d fast_ifetch_idle=%0d fast_ifetch_look=%0d fast_look_snoop=%0d fast_look_after_ce_resume=%0d ce_paused=%0d snoop_cycles=%0d cinv_cycles=%0d", iread_req_count, iread_ack_count, iread_mem_count, idle_path_cycles, look_path_cycles, fast_idle_accepts, fast_look_accepts, fast_look_snoop_edges, fast_look_after_ce_resume, ce_paused_cycles, snoop_cycles, cinv_cycles);
