#!/usr/bin/env bash
# Prepared P163 shared paired-read/write RAM port fit. NOT LAUNCHED.
# Run only after root deliberately promotes the P161 core and P151 cache patches in THIS
# checkout. It never applies a patch, creates a worktree, or changes production HDL.
set -u
repo=/home/alans/mister/MacQuadra800_MiSTer
fit_out="$repo/scratch/p163_combined_forward_shared_port_fit_20260921"
cd "$repo" || exit 1

if pgrep -x 'quartus_.*' >/dev/null 2>&1; then
  echo 'Quartus process already active; refusing to start P163.' >&2
  exit 3
fi
...
mkdir -p "$fit_out"
printf '%s\n' 'P163 development fit: CDROM_OFF and ETHERNET_OFF; no hardware deployment.' > "$fit_out/FEATURES.txt"
...
[[ "$actual_core" == a0d6053901b044128d9c6ff1513f19d2cc9318ad3285b1ddd226809022359770 ]] || { echo 'P161 core identity mismatch' >&2; exit 2; }
...
if [[ -f output_files/MacQuadra800.rbf && output_files/MacQuadra800.rbf -nt "$fit_out/start.stamp" ]]; then
  rbf="$fit_out/MacQuadra800_p163_combined_forward_shared_port_seed25_${fit_commit:0:7}.rbf"
