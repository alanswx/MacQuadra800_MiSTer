#!/usr/bin/env python3
"""Check pending RF-write overlap at indexed-memory admission and precise faults."""
...
integer count=0,overlap=0;
always @(posedge tb_ap040_program.clk)
 if(tb_ap040_program.nreset && tb_ap040_program.dut.core.ce && tb_ap040_program.dut.core.pipe_load_launch && tb_ap040_program.dut.core.pipe_load_pc==32'h602) count=count+1;
always @(posedge tb_ap040_program.clk)
 if(tb_ap040_program.nreset && tb_ap040_program.dut.core.ce && tb_ap040_program.dut.core.pipe_claim && tb_ap040_program.dut.core.pipe_input && tb_ap040_program.dut.core.pipe_ready && tb_ap040_program.dut.core.rf_we) overlap=overlap+1;
final begin
 $display("ENTRY RF overlaps=%0d",overlap);
 if(overlap!=3) $fatal(1,"missing pending RF write at admission");
...
'''
 ...
 log=(d/(name+'.log')).read_text();assert 'ALL TESTS PASSED' in log and 'FAIL:' not in log and 'INDEXED FAULT launches=3' in log and 'ENTRY RF overlaps=3' in log,log[-3000:]
