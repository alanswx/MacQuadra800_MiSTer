...
MMU is disabled. Synthetic exact operands, not the full Whetstone workload.
"""
import argparse
import hashlib
import json
...
subprocess.run([sys.executable, str(ROOT/'scripts/cpu/profile_sane_add.py'), str(a.rom.resolve()), '--out', str(d), '--prepare-only'], check=True)
...
identity.update(scope=__doc__, pipeline=True, mmu=False, flags=flags,
...
for latency in a.latencies:
    result = run([d/'obj/Vtb_cpu_sane', '+prog='+str(d/'program.hex'), f'+latency={latency}'], f'run32_latency{latency}.log')
    assert 'SANE_ADD PASS' in result
    print('\n'.join(x for x in result.splitlines() if x.startswith(('SANE_', 'LATENCY'))), flush=True)
...
negative = run([d/'obj/Vtb_cpu_sane', '+prog='+str(d/'negative.hex'), '+latency=3'], 'negative32.log', expect_failure=True)
