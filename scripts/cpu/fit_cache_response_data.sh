#!/usr/bin/env bash
# Prepared P147 seed-24 fit wrapper. NOT LAUNCHED.
# Run only after root deliberately applies/promotes the P147 cache patch in THIS
# checkout. It never applies a patch, creates a worktree, or changes production HDL.
set -u
repo=/home/alans/mister/MacQuadra800_MiSTer
fit_out="$repo/scratch/p147_cache_response_data_fit_20260921"
cd "$repo" || exit 1

if pgrep -x 'quartus_.*' >/dev/null 2>&1; then
  echo 'Quartus process already active; refusing to start P147.' >&2
  exit 3
fi
...
mkdir -p "$fit_out"
printf '%s\n' 'P147 development fit: CDROM_OFF and ETHERNET_OFF; no hardware deployment.' > "$fit_out/FEATURES.txt"
fit_commit=$(git rev-parse HEAD)
printf '%s\n' "$fit_commit" > "$fit_out/commit.txt"

# The promoted P147 source must already be in this checkout. The patch file is
...
[[ "$actual_cache" == 415efd9b2b96eb0972e6f3f75bf8b7ecd85e43b9e2aa78313de9256f1ff54aa3 ]] || { echo 'P147 cache is not promoted in this checkout' >&2; exit 2; }
...
if [[ -f output_files/MacQuadra800.rbf && output_files/MacQuadra800.rbf -nt "$fit_out/start.stamp" ]]; then
  rbf="$fit_out/MacQuadra800_p147_cache_response_data_seed24_${fit_commit:0:7}.rbf"
...
if [[ "$fit_ready" -eq 1 && -n "${QUARTUS_BIN:-}" ]]; then
  "$QUARTUS_BIN/quartus_sta" -t scripts/cpu/timequest_cross_domain.tcl p147_cache_response_data_seed24 > "$fit_out/cross.log" 2>&1
  cross_rc=$?
...
copy_fresh scratch/cross_sys2ram_p147_cache_response_data_seed24.txt "$fit_out/cross_sys2ram.txt"
copy_fresh scratch/cross_ram2sys_p147_cache_response_data_seed24.txt "$fit_out/cross_ram2sys.txt"
