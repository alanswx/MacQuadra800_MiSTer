...
import os
repo=pathlib.Path(os.environ.get('MQ_REPO','/home/alans/mister/MacQuadra800_MiSTer'))
...
with (here/'assemble.log').open('w')as f:
 subprocess.run([os.environ.get('VASM','/tmp/wombat-vasm/vasmm68k_mot'),'-Fbin','-m68040','-no-opt','-o',str(here/'program.bin'),str(here/'program.s')],check=True,stdout=f,stderr=subprocess.STDOUT)
...
with (here/'compile.log').open('w')as f:
 subprocess.run([os.environ.get('VERILATOR','/home/alans/verilator5/bin/verilator'),'--binary','--timing','--assert','-Wno-fatal','-j','4','--top-module','tb_latency','--Mdir',str(here/'obj'),'-I'+str(ap),*map(str,sources)],check=True,stdout=f,stderr=subprocess.STDOUT)
