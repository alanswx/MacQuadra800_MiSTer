...
always @(posedge clk) begin
    if (nreset && mem_ready && busstate == 2'b11) begin
        if (ram_sel) begin
            if (!nuds) begin
                mem[addr_out[21:1]][15:8] = data_write[15:8];
                if (addr_out >= RESULTS && addr_out < RESULTS + RES_LEN &&
                    data_write[15:8] == 8'h0A) begin
                    result_rows = result_rows + 1;
                    if (result_rows == 100) result = 1;
                end
            end
            if (!nlds) begin
                mem[addr_out[21:1]][7:0] = data_write[7:0];
                if (addr_out >= RESULTS && addr_out < RESULTS + RES_LEN &&
                    data_write[7:0] == 8'h0A) begin
                    result_rows = result_rows + 1;
                    if (result_rows == 100) result = 1;
                end
            end
