...
1. Build 6 is the deployable candidate (done: rbf copied, brief written); run the Opus operator on it once the box is parked, then write section 17/18. The routability switch must be added to the committed qsf (with a ledger note) if build 6's structure is kept. Earlier text: if a later build fails, copy the rbf to
