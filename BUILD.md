...
- `quartus_map` standalone skips the pre-flow script that regenerates
  `build_id.v`, which `MacQuadra800.sv` includes. `build_only.sh --check` invokes
  `sys/build_id.tcl` explicitly first, so it also works in a fresh clone, then
  maps with project settings read-only. The full flow runs the same generator
  through the QSF pre-flow hook.
